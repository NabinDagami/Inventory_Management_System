...
class InventoryView:
    ...
    def create_inventory_view(self):
        ...
        self.brands_btn.pack(side="left")
    ...
    def create_summary_cards(self, parent):
        ...
        ctk.CTkLabel(self.low_stock_card, text="⚠️ Low", font=ctk.CTkFont(size=10), text_color="white").pack(pady=(5, 0))
    ...
    def create_products_table(self, parent):
        ...
        columns = ("SKU", "Name", "Category", "Brand", "Stock", "Normal Price", "Workshop Price", "Reorder Level")
        ...
        column_widths = {"SKU": 140, "Name": 300, "Category": 160, "Brand": 160, 
                        "Stock": 120, "Normal Price": 140, "Workshop Price": 160, "Reorder Level": 140}
        
        for col in columns:
            self.data_tree.heading(col, text=f"  {col}  ", anchor="center")
            self.data_tree.column(col, width=column_widths.get(col, 120), anchor="center", minwidth=100)
        ...
        v_scrollbar.pack(side="right", fill="y", pady=2)
        ...
        self.context_menu.add_command(label="✏️  Edit", command=self.edit_item)
        ...
        self.context_menu.add_command(label="🗑️  Delete", command=self.delete_item, foreground="#EF4444")
    ...
    def switch_tab(self, tab_name):
        ...
        self.add_btn.configure(text=button_texts[tab_name])
    ...
    def filter_data(self, *args):
        ...
        for item in self.all_data:
            if self.current_tab == "products":
                ...
                # Apply all filters
                if matches_search and matches_category and matches_brand and matches_stock:
                    # Stock display with status icon and label
                    if stock == 0:
                        stock_display = "0 🔴 Out"
                    elif stock <= reorder_level:
    ...
    def add_product(self):
        ...
        if dialog.result:
            try:
                ...
                # Insert product
                db.execute_insert(
                    """INSERT INTO products (name, sku, category_id, brand_id, description, 
                                           stock, price_normal, price_workshop, cost_price, reorder_level)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (dialog.result['name'], sku, dialog.result['category_id'], dialog.result['brand_id'],
                ...
                try:
                    db.execute_update(
                        """UPDATE products SET name=?, category_id=?, brand_id=?, description=?,
                                             stock=?, price_normal=?, price_workshop=?, cost_price=?, reorder_level=?
                           WHERE sku=?""",
                        (dialog.result['name'], dialog.result['category_id'], dialog.result['brand_id'],
                         dialog.result['description'], dialog.result['stock'], dialog.result['price_normal'],
...
class ProductDialog:
    ...
    def create_form(self, product_data):
        ...
        # Category with searchable selection button
        ...
        # Brand with searchable selection button
        ...
        brand_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
            ...
            if product_data['category_id']:
                for cat in self.categories:
                    if cat['category_id'] == product_data['category_id']:
                        self.category_var.set(cat['category_name'])
                        break
            
            if product_data['brand_id']:
                for brand in self.brands:
                    if brand['brand_id'] == product_data['brand_id']:
                        self.brand_var.set(brand['brand_name'])
                        break
    ...
    def show_category_selector(self):
        """Show searchable category selection dialog"""
    ...
    def show_brand_selector(self):
        """Show searchable brand selection dialog"""
    ...
    def show_category_dropdown(self, event=None):
        """Show category dropdown list"""
        self.category_listbox.delete(0, tk.END)
        for cat in self.categories:
            self.category_listbox.insert(tk.END, cat['category_name'])
        self.category_listbox.pack(anchor="w", padx=20, pady=(0, 10), fill="x")
    
    def filter_category_dropdown(self, event=None):
        """Filter category dropdown based on typed text"""
        search_term = self.category_entry.get().lower()
        self.category_listbox.delete(0, tk.END)
        for cat in self.categories:
            if search_term in cat['category_name'].lower():
                self.category_listbox.insert(tk.END, cat['category_name'])
    ...
    def on_category_select(self, event=None):
        ...
        if selection:
            selected_name = self.category_listbox.get(selection[0])
            self.category_entry.delete(0, tk.END)
            self.category_entry.insert(0, selected_name)
            for cat in self.categories:
                if cat['category_name'] == selected_name:
                    self.category_id_var.set(cat['category_id'])
                    break
            self.category_listbox.pack_forget()
    
    def show_brand_dropdown(self, event=None):
        """Show brand dropdown list"""
        self.brand_listbox.delete(0, tk.END)
        for brand in self.brands:
            self.brand_listbox.insert(tk.END, brand['brand_name'])
        self.brand_listbox.pack(anchor="w", padx=20, pady=(0, 10), fill="x")
    
    def filter_brand_dropdown(self, event=None):
        """Filter brand dropdown based on typed text"""
        search_term = self.brand_entry.get().lower()
        self.brand_listbox.delete(0, tk.END)
        for brand in self.brands:
            if search_term in brand['brand_name'].lower():
                self.brand_listbox.insert(tk.END, brand['brand_name'])
        if self.brand_listbox.size() > 0:
            self.brand_listbox.pack(anchor="w", padx=20, pady=(0, 10), fill="x")
        else:
            self.brand_listbox.pack_forget()
    
    def on_brand_select(self, event=None):
        """Handle brand selection from dropdown"""
        selection = self.brand_listbox.curselection()
        if selection:
            selected_name = self.brand_listbox.get(selection[0])
            self.brand_entry.delete(0, tk.END)
            self.brand_entry.insert(0, selected_name)
            for brand in self.brands:
                if brand['brand_name'] == selected_name:
                    self.brand_id_var.set(brand['brand_id'])
                    break
            self.brand_listbox.pack_forget()
    
    def hide_dropdown_after_delay(self, dropdown_type):
        """Hide dropdown after a short delay to allow click events"""
        self.dialog.after(200, lambda: self.hide_dropdown(dropdown_type))
    
    def hide_dropdown(self, dropdown_type):
        """Hide the specified dropdown"""
        if dropdown_type == 'category':
            if not self.category_listbox.focus_get() == self.category_listbox:
                self.category_listbox.pack_forget()
        elif dropdown_type == 'brand':
            if not self.brand_listbox.focus_get() == self.brand_listbox:
                self.brand_listbox.pack_forget()
    ...
    def save_product(self):
        ...
        try:
            ...
            category_id = self.category_id_var.get() if self.category_id_var.get() else None
            brand_id = self.brand_id_var.get() if self.brand_id_var.get() else None
...
class CategoryDialog:
    ...
    def create_form(self, category_data):
        ...
        category_icon = ctk.CTkLabel(header_frame, text="🏷️", font=ctk.CTkFont(size=24))
...
class BrandDialog:
    ...
    def create_form(self, brand_data):
        ...
        brand_icon = ctk.CTkLabel(header_frame, text="🏢", font=ctk.CTkFont(size=24))
        brand_icon.pack(pady=(10, 5))
